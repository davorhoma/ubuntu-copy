...
#pragma omp parallel default(none) shared(                                     \
        seed, dailyKey, overCnt, totalRevenue, poslednjiKod, paketaPoKuriru)
    {
#pragma omp single
        {
#pragma omp task firstprivate(seed) shared(dailyKey)
            {
                dailyKey = izracunaj_dnevni_kljuc(seed);
            }
        }

#pragma omp for reduction(+ : overCnt, totalRevenue) lastprivate(poslednjiKod)
        for (int i = 0; i < BROJ_POSILJAKA; i++) {
            int m = gen_mass(seed, i);
            int z = gen_zone(seed, i);
            ...
            // poslednja iteracija (i=N-1) daje poslednjiKod -> potreban kasnije
            poslednjiKod = napravi_scan_code(seed, i, m, z, price);
        }
    }

    // posle parallel-a: dailyKey je gotov (implicitna barijera, ukoliko nema
    // nikakve implicitne, trebalo bi #pragma omp taskwait), a poslednjiKod je
    // iz lastprivate
