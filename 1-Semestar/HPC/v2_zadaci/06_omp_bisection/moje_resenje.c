...
void find_roots(double (*f)(double), double a, double b, double *xroot, int maxiters, double tol, double step) {
    double xp = a, xk = b;
    double fabs_val = fabs(xk - xp);
    double dx = fabs_val / 2;
    double xroot1, xroot2;
    
    #pragma omp parallel firstprivate(xp, xk, dx, xroot1, xroot2, xroot)
    #pragma omp single
    {
        if (fabs_val < step) {
            double bisection_val;
            #pragma omp task firstprivate(bisection_val)
            {
                bisection_val = bisection(f, xp, xk, xroot, maxiters, tol);
                if (bisection_val) {
...
void find_roots_parallel(double (*f)(double), double a, double b, double *xroot, int maxiters, double tol, double step) {
    ...
    for (int i = fabs_val; i < step; i = fabs(xk - xp)) {
    ...
    // find_roots_parallel(f, xp, xp + dx, &xroot1, maxiters, tol, step);
    // find_roots_parallel(f, xp + dx, xk, &xroot2, maxiters, tol, step);
}
...
int main() {
    ...
    // #pragma omp parallel private(xroot) num_threads(8)
    // #pragma omp single
